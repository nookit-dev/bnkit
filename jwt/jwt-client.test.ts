...
describe("jwtClientSideFactory", () => {
  ...
  test("decodeJwt decodes a JWT", () => {
    ...
    expect(decodedToken.payload).toEqual({ sub: "1234567890", name: "John Doe", roles: ["admin"], exp: 1609459200 });
  });

  test("isJwtExpired checks if a JWT is expired", () => {
    const expiredToken = base64url(Buffer.from('{"alg":"HS256","typ":"JWT"}')) + '.' + base64url(Buffer.from('{"sub":"1234567890","name":"John Doe","roles":["admin"],"exp":1609459200}'));
    const unexpiredToken = base64url(Buffer.from('{"alg":"HS256","typ":"JWT"}')) + '.' + base64url(Buffer.from('{"sub":"1234567890","name":"John Doe","roles":["admin"],"exp":1909459200}'));
