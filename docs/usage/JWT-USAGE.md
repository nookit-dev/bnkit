...
import {
  ...
  encrypt,
  encodeJwt,
} from "@bnk/core/modules/jwt";
import type { JwtHeader, JwtPayload, RefreshToken } from "@bnk/core/modules/jwt";
```
...
With these tools, you're well-equipped to handle JWTs on the backend with TypeScript. Remember to handle tokens securely, manage expirations appropriately, and leverage TypeScript's type system for robust and error-free code. Happy coding! 🚀🔒💻## Usage Guide: JSON Web Tokens (JWT) in TypeScript (Backend)
